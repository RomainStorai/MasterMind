...
void calculer_points() {
	...
	// Compter les ronds
	for (int i=0; i<4;i++) {
		if (tabnbCercle[i][1] == tabnbCercle[0][0] || tabnbCercle[i][1] == tabnbCercle[1][0] || tabnbCercle[i][1] == tabnbCercle[2][0] || tabnbCercle[i][1] == tabnbCercle[3][0]) {
...
void partie() {
	...
	if (nbCercle > 3) {
		printf(GRN "\t\tFelicitations, c'est gagne !\n\tVous avez trouve la bonne combinaison en %d essais !", nbTour - 1);
